...
from gtk_compat import gtk, pangocairo
import cairo
import struct
import os
...
def _resolve_monospace_font(font_name):
    name = (font_name or '').strip()
    lower = name.lower()
    candidates = []
    if lower in ('', 'monospace', 'mono'):
        candidates.extend([
            'Source Code Pro',
            'Consolas',
            'Cascadia Mono',
        ...
        if lower not in ('monospace', 'mono'):
            candidates.append('Monospace')

    try:
        families = pangocairo.FontMap.get_default().list_families()
        available = {f.get_name().lower(): f.get_name() for f in families}
        for candidate in candidates:
            chosen = available.get(candidate.lower())
            if chosen:
                return chosen
    except Exception:
        pass

    return candidates[0] if candidates else 'Monospace'
...
class HexView():
    ...
    def set_dxdy(self):
        ...
        ctx.select_font_face(_resolve_monospace_font(self.font), cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
...
#           self.vadj.upper = self.lines-self.numtl+2
    ...
    def expose(self,widget,event):
        ...
        # GTK3 queue_draw triggers a full repaint on a fresh surface;
        # the old partial-update path only painted cursor deltas and left
        # the rest blank. Always redraw the full visible area.
        if True:
...
# hex/asc  part
        ...
        if self.prer-self.offnum > -1:
            ...
            if self.prec > -1 and self.prec < 16:
                ...
                ctx.show_text("%02x " % _byte_value(self.data[self.prer*16+self.prec]))
                # location of asc
        ...
        if self.curr-self.offnum > -1:
            ...
            ctx.select_font_face(_resolve_monospace_font(self.font), cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_BOLD)
            ctx.show_text("%02x " % _byte_value(self.data[self.curr*16+self.curc]))
            ctx.select_font_face(_resolve_monospace_font(self.font), cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
            ctx.set_source_rgb(0,0,0)
