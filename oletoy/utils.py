...
def _to_gtk_string(value):
    if value is None:
        return None
    if isinstance(value, bytes):
        return value.rstrip(b"\x00").decode("latin1", "replace")
...
def add_iter(hd, name, value, offset, length, vtype, offset2=0, length2=0, parent=None, tip=None):
    ...
    tip: optional additional explanation of the entry.
    """
    iter = hd.model.append(parent, None)
    hd.model.set(iter,
        0, _to_gtk_string(name),
        1, _to_gtk_string(value),
        2, offset,
        3, length,
        4, _to_gtk_string(vtype),
        5, offset2,
        6, length2,
        8, _to_gtk_string(tip))
    return iter
...
def d2hex(data,space="",ln=0):
    ...
    for i in range(len(data)):
        ch = data[i]
        chv = ch if isinstance(ch, int) else ord(ch)
        s += "%02x%s"%(chv,space)
        if ln != 0 and i > 0 and (i+1)%ln == 0:
            s += "\n"
    return s
...
def d2bin(data):
    vals = []
    for x in data:
        xv = x if isinstance(x, int) else ord(x)
        vals.append(format(xv, 'b').zfill(8))
    return ' '.join(vals)
...
def graph_expose (da,event,data,hd):
    ...
    for x in range(len(data)):
        b = data[x]
        y1 = b if isinstance(b, int) else ord(b)
#       if y1 - y < 10:
