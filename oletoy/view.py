...
from gtk_compat import gobject
from gtk_compat import gtk, pango
import cairo
import tree
import uniview
import hexdump
...
escher = optional_import('escher')
quill = optional_import('quill')
vsd = optional_import('vsd')
vsd2 = optional_import('vsd2')
vsdchunks = optional_import('vsdchunks')
vsdchunks5 = optional_import('vsdchunks5')
vsdstream4 = optional_import('vsdstream4')
xls = optional_import('xls')
vba = optional_import('vba')
ole = optional_import('ole')
doc = optional_import('doc')
mdb = optional_import('mdb')
pub = optional_import('pub')
ppt = optional_import('ppt')
rtf = optional_import('rtf')
pm6 = optional_import('pm6')
qxp = optional_import('qxp')
emfparse = optional_import('emfparse')
svm = optional_import('svm')
mf = optional_import('mf')
wmfparse = optional_import('wmfparse')
emfplus = optional_import('emfplus')
rx2 = optional_import('rx2')
fh = optional_import('fh')
fh12 = optional_import('fh12')
cdr = optional_import('cdr')
cmx = optional_import('cmx')
wld = optional_import('wld')
cpt = optional_import('cpt')
ppp = optional_import('ppp')
pict = optional_import('pict')
chdraw = optional_import('chdraw')
yep = optional_import('yep')
midi = optional_import('midi')
riff = optional_import('riff')
dsf = optional_import('dsf')
drw = optional_import('drw')
vfb = optional_import('vfb')
lrf = optional_import('lrf')
wls = optional_import('wls')
wt602 = optional_import('wt602')
...
class ApplicationMainWindow(gtk.Window):
    def __init__(self, parent = None):
        ...
        for gsf_name in gsf_candidates:
            try:
                self.cgsf = ctypes.cdll.LoadLibrary(gsf_name)
                self.gsfname = gsf_name
                break
            except Exception:
                pass
        if self.cgsf is None:
            print("Libgsf not found. OLE-based files will be unavailable until a libgsf DLL is added to the GTK/runtime PATH.")
    ...
    def init_config(self): # redefine UI/behaviour options from file
        ...
        self.fontsize = 14
        self.hv_fontsize = 14
        self.gsfname = 'libgsf-1-114.dll' if os.name == 'nt' else 'libgsf-1.so'
...
<b>Diff:</b>\n\
    ...
    def calc_status(self,buf,dlen):
        ...
        if ftype == "YEP":
            if dlen == 1:
                b = buf if isinstance(buf, int) else ord(buf[:1])
                txt += key2txt(b,midi.pitches,"")
    ...
    def edited_cb (self, cell, path, new_text):
        ...
        if fmt == "clr":
            ...
            fmt_code = fmt[-1] if fmt else ""
            if fmt_code in ("f", "d", "e"):
                num_value = float(new_text)
            else:
                num_value = int(new_text, 0)
            value = value[0:offset] + struct.pack(fmt, num_value) + value[offset+size:]
    ...
    def file_open (self, title='Open', parent=None, dirname=None, fname=""):
        if os.name == 'nt' and filedialog is not None and Tk is not None:
            root = None
            try:
                root = Tk()
                root.withdraw()
                root.attributes('-topmost', True)
                initialdir = dirname or None
                initialfile = fname or None
                if title == 'Save':
