...
try:
    from gtk_compat import gtksourceview2
    usegtksv2 = True
except:
    usegtksv2 = False
...
def xlsfind(model, path, iter, user_data):
    page, rowaddr, coladdr = user_data
...
def recfind(model, path, iter, user_data):
    page, data = user_data
...
def cmdfind(model, path, iter, user_data):
    page, data = user_data
...
#               print "Failed to add as a picture"
            ...
            try:
                ...
                try:
                    f = StringIO.BytesIO(buf[int(chaddr,16):])
