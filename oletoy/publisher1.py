...
class Publisher1Doc():
    ...
    def BorderArts(self,length,off):
        ...
        return ("BorderArts", 'pub', length, subZone)
    ...
    def TextSp(self,length,off,wh):
        ...
        for i in range(N):
            ...
            span_len=struct.unpack('<B', self.data[begPos+2*offset:begPos+2*offset+1])[0]
            if wh=="TextSpan":
                subZone.append(("%s%x"%(wh,offset),wh,2*offset,1+span_len))
            else:
                subZone.append(("%s%x"%(wh,offset),wh,2*offset,1+2*span_len))
        return subZone
    ...
    def hdZoneShape(self,hd,data):
        ...
        for j in range(2):
            ...
            if j % 2==0:
                dims+="x"
            off+=2
        add_iter (hd,"size",dims,off-4,4,'txt')
