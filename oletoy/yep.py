...
def _b2s(v):
    return v.decode("latin1", "replace") if isinstance(v, (bytes, bytearray)) else v
...
def bank (hd, data, off):
    ...
    offset = 61
    x = 128-_bval(data[offset])
    add_iter(hd,"Envelope - Attack",x,offset,1,"B")

    offset = 62
    x = 127-_bval(data[offset])
...
def hdrbch (hd, data, off):
    ...
    x = _bval(data[offset])        
    if x >=32:
                x = _bval(data[offset])-128
    add_iter(hd,"Tuning Coarse",x,offset,1,"B")

    offset = 18
    x = _bval(data[offset])        
    if x >=64:
                x = _bval(data[offset])-256
...
def hdr1item (page,data,parent,offset=0):
    ...
    off += 4
    vdtxt = "Drumkit"
    if _bval(data[0x25]) == 0x00:
        vdtxt = "Voice"
    vdidxa = _bval(data[0x21])
    vdidxb = _bval(data[0x22])
    vdidxc = _bval(data[0x23])    
    ...
    for i in slist:
        ...
        try:  # to workaround current problem with Europack
            for j in range(i[0],i[1]+1):
                if not j < 0:
                    bend = hdrb[j]
                    v3 = _bval(data[off2+9])
                    v4 = _bval(data[off2+8])
...
def parse (page, data, parent,align=4.,prefix="",offset=0):
    ...
    while off < len(data):
        ...
        off += 4
        if fourcc == "SSTY":
            stname = _zstr(data[off:off+16])
            iname = fourcc+" %s"%stname
            if sstygrpiter == None:
                sstygrpiter = add_pgiter(page,"SSTYs","ssty","dontsave","",parent,"%02x  "%(offset+off))
            piter = sstygrpiter
        elif fourcc == "VVST":
            na = _bval(data[off+0x18])
            nb = _bval(data[off+0x19])
            nc = _bval(data[off+0x1a])
            if _bval(data[off+0x14]) == 0x3f or _bval(data[off+0x14]) == 0x00:
                f = "[Voice %d-%d-%d]"%(na,nb,nc)
            else:
                f = "[DrumKit %d-%d-%d]"%(na,nb,nc)
            iname = fourcc+f+" %s"%(_zstr(data[off:off+16]))
            if vvstgrpiter == None:
