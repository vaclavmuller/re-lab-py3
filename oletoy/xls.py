...
def biff_cfex (hd,data):
    ...
    if fIsCF12 == 0:
        ...
        off += 2
        cp = data[off] if isinstance(data[off], int) else ord(data[off])
        add_iter(hd,"cp",cp,off,1,"B")
        off += 1
        icfTemplate = data[off] if isinstance(data[off], int) else ord(data[off])
        ...
        off += 2
        flags = data[off] if isinstance(data[off], int) else ord(data[off])
        fActive = flags & 1
        fStopIfTrue = flags & 2
        add_iter(hd,"fActive",fActive,off,1,"B")
        add_iter(hd,"fStopIfTrue",fStopIfTrue,off,1,"B")
        off += 1
        fHasDxf = data[off] if isinstance(data[off], int) else ord(data[off])
        add_iter(hd,"fHasDxf",fHasDxf,off,1,"B")
        off += 1
        if fHasDxf == 1:
            off = biff_dxfn12(hd,data,off)
        cbTemplateParm = data[off] if isinstance(data[off], int) else ord(data[off])
