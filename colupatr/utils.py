...
import struct
from gtk_compat import gtk, gobject
from pycompat import bval, ensure_text, ensure_bytes
...
def arg_conv (ctype,carg):
	...
	if ctype.lower() == 'x':
		...
		data = ensure_bytes(carg).encode("utf-16")[2:]
	elif ctype.lower() == 'a' or ctype.lower() == 'r':
		data = ensure_bytes(carg)
	return data
		...
		while off < dlen:
			...
			if len(cmntest) > 0:
				...
				for cmnt in cmntest:
					...
					tmpoff = addr2
					txtcmnt += "<span style='color: rgb(%s);'>"%cmntclr+doc.comments[cmnt].text+"</span> "+"·" + " "
				txthex += " " + txt1[addr2*3:]
				txtasc += txt2[addr2:]
