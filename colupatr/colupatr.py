...
from gtk_compat import gtk, gobject, pangocairo, GLib
from pycompat import bval, ensure_text, ensure_bytes, exec_config, exec_script
import hexview
import utils
import cli
import midi
...
# Suppress PyGObject deprecation warnings from legacy Gtk.UIManager calls
# kept for compatibility in the current port.
warnings.filterwarnings("ignore", message=r".*Gtk\.UIManager\.get_accel_group is deprecated.*", category=DeprecationWarning)
warnings.filterwarnings("ignore", message=r".*Gtk\.UIManager\.get_widget is deprecated.*", category=DeprecationWarning)
...
def register_stock_icons():
	'''Legacy hook kept for compatibility. Stock icon registration is not needed in the PyGObject port.'''
	return
...
class ApplicationMainWindow(gtk.Window):
	def __init__(self, parent=None):
		...
		self.config_dir = self.snipsdir
		self.options_path = os.path.join(self.config_dir, "options.cfg")
		...
		self.ensure_default_options_config()
		try:
			exec_config(self.options_path, self)
		except Exception as exc:
			print("Options config not loaded:", exc)
		self.run_win = None
	...
	def ensure_default_options_config(self):
		legacy_path = os.path.join(os.getcwd(), "options.cfg")
		if not os.path.exists(self.options_path) and os.path.exists(legacy_path):
			try:
				with open(legacy_path, "rb") as src, open(self.options_path, "wb") as dst:
					dst.write(src.read())
			except Exception as exc:
				print("Failed to migrate legacy options.cfg:", exc)
		if os.path.exists(self.options_path):
			return
		self.write_options_config()

	def write_options_config(self):
		ensure_dir(self.config_dir)
		with open(self.options_path, "w", encoding="utf-8") as f:
			...
			f.write("self.options_enc = %r\n" % self.options_enc)
			f.write("self.options_htmlhdr = %s\n" % self.options_htmlhdr)

	def file_open_native_windows(self, title='Open', fname=""):
		if os.name != 'nt':
			return None
		try:
			...
			print("tkinter file dialog unavailable:", exc)
			return None
		root = tk.Tk()
		root.withdraw()
		try:
		...
		try:
			if title == 'Save':
				path = filedialog.asksaveasfilename(initialfile=fname or '', title='Save...')
			else:
	...
	def draw_manual (self, widget, ctx):
		mytxt = """<b>Keys:</b>
...
<b>Runs:</b>
	...
	<tt>run if cli.rwrap(hv,hv.line_size(hv.curr+1)): cli.prev(hv)</tt>
		...
		layout.set_markup(mytxt)
		w, h = layout.get_size()
		widget.set_size_request(int(w / 1000), int(h / 1000))
		ctx.move_to(0, 0)
	...
	def activate_export(self, action):
			...
			self.expwin = gtk.Window(type=gtk.WINDOW_TOPLEVEL)
	...
	def on_options_save(self,action):
		try:
			f = open("options.cfg","w", encoding="utf-8")
			f.write("# options for colupatr\n")
			f.write("self.options_le = %s\n"%self.options_le)
			f.write("self.options_be = %s\n"%self.options_be)
			f.write("self.options_txt = %s\n"%self.options_txt)
	...
	def activate_options (self, action):
		if self.options_win != None:
			...
			optwin = gtk.Window(type=gtk.WINDOW_TOPLEVEL)
	...
	def rlp_pack(self,dscr,fmt,value,f):
		...
		f.write(ensure_bytes(dscr))
		f.write(ensure_bytes(fmt))
		if fmt == " s":
			vl = len(value)
			f.write(struct.pack("<I",vl))
			v = ensure_bytes(value)
		else:
	...
	def activate_save(self,action):
		...
		if pn != -1:
			...
			if fname:
				doc.fname = fname
				pname = os.path.basename(fname) or fname
				ebox = self.notebook.get_tab_label(doc.table)
				ebox.get_children()[0].set_text(pname)
				f = open(fname,"wb")
				f.write(b"RE-LABv05 [DL(B)|D|VF(2c)|VL(<I)|V]")
	...
	def rlp_unpack(self,buf,off):
		dl = bval(buf[off])
		off += 1
		k = ensure_text(buf[off:off+dl])
		off += dl
		fmt = ensure_text(buf[off:off+2])
		off += 2
		if fmt == " s":
			vl = struct.unpack("<I",buf[off:off+4])[0]
			off += 4
			v = ensure_text(buf[off:off+vl])
			off += vl
		else:
			...
			if buf == None:
				...
				if rbuf[:9] == b"RE-LABv05":
					...
					for i in range(v):
						tl = bval(rbuf[off])
						off += 1
						txt = ensure_text(rbuf[off:off+tl])
						off += tl
						coff = struct.unpack("<I",rbuf[off:off+4])[0]
						off += 4
						clen = struct.unpack("<I",rbuf[off:off+4])[0]
						off += 4
						clr0 = bval(rbuf[off])/255.0
						clr1 = bval(rbuf[off+1])/255.0
						clr2 = bval(rbuf[off+2])/255.0
						ct = bval(rbuf[off+3])
						off += 4
	...
	def file_open (self, title='Open', parent=None, dirname=None, fname=""):
		if os.name == 'nt':
			path = self.file_open_native_windows(title, fname)
			if path is not None:
				return path
		if title == 'Save':
			dlg = gtk.FileChooserDialog('Save...', parent, action=gtk.FILE_CHOOSER_ACTION_SAVE, buttons=(gtk.STOCK_OK,gtk.RESPONSE_OK,gtk.STOCK_CANCEL,gtk.RESPONSE_CANCEL))
	...
	def calc_status(self,buf,dlen):
		...
		txt = ""
		if dlen == 1:
			v1 = bval(buf[0] if isinstance(buf, (bytes, bytearray)) else buf)
			txt += "%d\t"%v1
			...
			if self.options_ipaddr == 1:
				txt += "%d.%d.%d.%d"%(bval(buf[0]),bval(buf[1]),bval(buf[2]),bval(buf[3]))
		elif dlen == 8:
			if self.options_le == 1:
			...
			txt = '<span background="#%02x%02x%02x">RGB</span>  '%(bval(buf[0]),bval(buf[1]),bval(buf[2]))
			txt += '<span background="#%02x%02x%02x">BGR</span>'%(bval(buf[2]),bval(buf[1]),bval(buf[0]))
		if dlen > 3 and self.options_txt == 1:
			try:
				if isinstance(buf, (bytes, bytearray)):
					decoded = buf.decode(self.options_enc, errors='replace')
				else:
					decoded = str(buf)
				decoded = decoded.replace("\n", "\\n")[:32]
				decoded = GLib.markup_escape_text(decoded)
				txt += '\t<span background="#DDFFDD">%s</span>' % decoded
			except Exception:
				pass
		...
		if len(cmdline) > 0:
			...
			if pn != -1:
				...
				if cmd[0].lower() == "name" and len(cmd) > 1:
					...
					import importlib; importlib.reload(globals()[cmd[1]])
...
#		addr = model1.get_value(iter1,1)
	...
	def show_search(self,carg):
		...
		scrolled.show()
		searchwin = gtk.Window(type=gtk.WINDOW_TOPLEVEL)
