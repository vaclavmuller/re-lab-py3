...
import cairo
from pycompat import bval, ensure_text, exec_config
import struct
import math
import utils, cli
...
class Comment():
	def __init__(self,text="",offset=0,length=0,color=(0,0,0),ctype=0):
		self.text = ensure_text(text)		# text of the comment
...
class HexView():
	def __init__(self,data=None,lines=[],comments={},offset=0):
		...
		try:
			self.vs.set_adjustment(self.vadj)
		except Exception:
			pass
		self.hbox1 = gtk.HBox()
		self.hbox2 = gtk.HBox()
		...
		self.ewin = gtk.Window(type=gtk.WINDOW_TOPLEVEL)
	...
	def chk_offset(self,offset):
		for i in sorted(self.comments.keys()):
			if i > offset:
				return -1
	...
	def _hex_x(self, ctx, col):
		"""Compute x position of hex cell using text extents, matching row rendering."""
		base = self.tdx*10
		if col <= 0:
			return base
		ctx.select_font_face(self.font, cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
		prefix = "00 " * col
		return base + ctx.text_extents(prefix).x_advance

	def _asc_x(self, ctx, col):
		base = self.tdx*(10+1+self.maxaddr*3)
		if col <= 0:
			return base
		ctx.select_font_face(self.font, cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
	...
	def _hex_col_from_x(self, x, row, allow_end=False):
		ctx = self._measure_ctx()
		ls = max(self.line_size(row), 0)
		if ls <= 0:
			return 0
		if x <= self._hex_x(ctx, 0):
			return 0
		for col in range(ls):
			start = self._hex_x(ctx, col)
			next_x = self._hex_x(ctx, col + 1)
			if x < next_x:
				return col
		return ls if allow_end else ls - 1

	def _asc_col_from_x(self, x, row, allow_end=False):
		ctx = self._measure_ctx()
		ls = max(self.line_size(row), 0)
		if ls <= 0:
			return 0
		if x <= self._asc_x(ctx, 0):
			return 0
		for col in range(ls):
			start = self._asc_x(ctx, col)
			next_x = self._asc_x(ctx, col + 1)
			if x < next_x:
				return col
		return ls if allow_end else ls - 1
	...
	def on_vadj_changed (self, vadj):
		...
		try:
			...
			value = int(getattr(vadj, 'value', 0))
		max_value = max(len(self.lines) - max(self.numtl, 1), 0)
		if value < 0:
			value = 0
		elif value > max_value:
			value = max_value
		if value != self.offnum:
			self.offnum = value
			self._sync_vscroll()
			self.hv.queue_draw()
		return True
	...
	def _sync_vscroll(self):
		# Keep scrollbar semantics close to the original PyGTK app:
		# the thumb represents the current top visible line, not a proportional viewport size.
		page_size = 1.0
		total = max(len(self.lines), 1)
		max_value = max(total - max(int(self.numtl), 1), 0)
		lower = 0.0
		upper = float(max_value + 1)
		step_inc = 1.0
		page_inc = float(max(max(int(self.numtl), 1) - 1, 1))
		if self.offnum < 0:
			self.offnum = 0
		elif self.offnum > max_value:
			self.offnum = max_value
		try:
			self.vadj.configure(float(self.offnum), lower, upper, step_inc, page_inc, page_size)
		except Exception:
			self.vadj.set_lower(lower)
			self.vadj.set_upper(upper)
			self.vadj.set_step_increment(step_inc)
			self.vadj.set_page_increment(page_inc)
			self.vadj.set_page_size(page_size)
			self.vadj.set_value(float(self.offnum))
		try:
			self.vs.set_adjustment(self.vadj)
		except Exception:
			pass
	...
	def set_dxdy(self):
		# calculate character extents without relying on the old PyGTK widget.window API
		surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, 1, 1)
		ctx = cairo.Context(surface)
		ctx.select_font_face(self.font, cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
		ctx.set_font_size(self.fontsize)
		ctx.set_line_width(1)
		(xt, yt, wt, ht, dx, dy) = ctx.text_extents("0")
		advance = max(dx, wt)
		self.tdx = int(math.ceil(advance))
		self.tht = int(math.ceil(ht+6))
		addr_w = int(math.ceil(ctx.text_extents('%08x' % 0).x_advance)) + self.tdx
		self.hbox1.set_size_request(addr_w,0)
	...
	def init_config(self): # redefine UI/behaviour options from file
		...
		try:
			exec_config("colupatr.cfg", self)
		except:
			try:
				exec_config("oletoy.cfg", self)
			except:
				pass
			try:
				exec_config(os.path.expanduser("~/.oletoy/oletoy.cfg"), self)
			except:
				pass
...
#				else:
	...
	def on_motion_notify (self, widget, event):
		if self.drag:
			...
			ctxm = self._measure_ctx()
			hex_left = self._hex_x(ctxm, 0)
			hex_right = self._hex_x(ctxm, self.maxaddr)
			asc_left = self._asc_x(ctxm, 0)
			asc_right = self._asc_x(ctxm, self.maxaddr)
			if event.x > hex_left:
				if event.x < hex_right: # hex
					colnum = self._hex_col_from_x(event.x, rownum, allow_end=True)
				elif event.x < asc_right: # ascii
					colnum = self._asc_col_from_x(event.x, rownum, allow_end=True) + 1
				else:
	...
	def on_button_press (self, widget, event):
		...
		if event.x > hex_left:
			if event.x < hex_right: # hex
				colnum = self._hex_col_from_x(event.x, rownum, allow_end=False)
			elif event.x < asc_right: #ascii
				colnum = self._asc_col_from_x(event.x, rownum, allow_end=False)
			else:
	...
	def expose (self, widget, ctx):
		if ctx is None or not hasattr(ctx, "set_source_surface"):
			try:
				self.hv.queue_draw()
			except Exception:
				pass
			return False
		orig_ctx = ctx
		width = self.hv.get_allocated_width()
		height = self.hv.get_allocated_height()
		self.cmntlines = {}

		if self.tdx == -1:
			self.set_dxdy()
		# With offscreen Cairo rendering, incremental PyGTK-style repainting leaves the new surface blank.
		# Force a full redraw on every expose/draw; performance is acceptable for this viewer.
		self.mode = ""
		# Guard against invalid initial sizes and avoid resize loops from set_size_request() inside draw.
		if width <= 1 or height <= 1 or self.tht <= 0:
			return False
		draw_width = max(1, min(int(width), 4096))
		draw_height = max(1, min(int(height), 4096))
		surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, draw_width, draw_height)
		ctx = cairo.Context(surface)
		ctx.select_font_face(self.font, cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
		ctx.set_font_size(self.fontsize)
		ctx.set_line_width(1)
		...
		height = draw_height
		cmnt_off = (-1,-1)
		self.numtl = min(max(int((height - self.tht-4)/self.tht)+1, 1), len(self.lines))
		self._sync_vscroll()
...
#hdr
			hdr = ""
			ctx.move_to(self._hex_x(ctx, self.curc),self.tht+1.5)
			ctx.line_to(self._hex_x(ctx, self.curc+1)-self.tdx,self.tht+1.5)
...
# hex/asc  part
		...
		ctx.set_source_rgb(0.9,0.9,0.9)
		ctx.move_to(self._hex_x(ctx, self.prec),self.tht+1.5)
		ctx.line_to(self._hex_x(ctx, self.prec+1)-self.tdx,self.tht+1.5)
		ctx.stroke()
		...
		ctx.line_to(self._hex_x(ctx, self.curc)+self.tdx*2,self.tht+1.5)
		ctx.stroke()
		...
		if self.prer-self.offnum > -1:
			...
			if self.sel:
				if self.sel[0] == self.sel[2]:
					...
					if self.prer == self.sel[0]:
						...
						ctx.set_source_rgb(0.7,0.9,0.8)
			if self.prer > -1 and self.prer < len(self.lines)-1 and self.prec > -1 and self.prec < (self.lines[self.prer+1][0]-self.lines[self.prer][0]):
				# old hex char
				ctx.rectangle(self._hex_x(ctx, self.prec),(self.prer-self.offnum+1)*self.tht+6.5,self.tdx*2+2,self.tht)
				# old asc char
				ctx.rectangle(self._asc_x(ctx, self.prec),(self.prer-self.offnum+1)*self.tht+6,self.tdx+2,self.tht+1.5)
				ctx.fill_preserve()
			...
			if self.prer > -1 and self.prer < len(self.lines)-1 and self.prec > -1 and self.prec < (self.lines[self.prer+1][0]-self.lines[self.prer][0]):
				# location of hex
				ctx.move_to(self._hex_x(ctx, self.prec),(self.prer-self.offnum+2)*self.tht+4)
				ctx.show_text("%02x "%bval(self.data[self.lines[self.prer][0]+self.prec]))
				# location of asc
				ctx.move_to(self._asc_x(ctx, self.prec),(self.prer-self.offnum+2)*self.tht+4)
		...
		if self.curr-self.offnum > -1:
			ctx.rectangle(self._hex_x(ctx, self.curc),(self.curr-self.offnum+1)*self.tht+6.5,self.tdx*2+2,self.tht)
			ctx.set_source_rgb(1,1,1)
			if self.sel:
				if self.sel[0] == self.sel[2]:
					...
					if self.curr == self.sel[0]:
						...
						ctx.set_source_rgb(0.7,0.9,0.8)
			ctx.fill()
			ctx.rectangle(self._asc_x(ctx, self.curc),(self.curr-self.offnum+1)*self.tht+6,self.tdx+1,self.tht+1)
			ctx.set_source_rgb(0.75,0.75,1)
			ctx.fill()
			ctx.set_source_rgb(0,0,1)
			ctx.move_to(self._hex_x(ctx, self.curc),(self.curr-self.offnum+2)*self.tht+4)
			ctx.select_font_face(self.font, cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_BOLD)
			ctx.show_text("%02x "%bval(self.data[self.lines[self.curr][0]+self.curc]))
			ctx.select_font_face(self.font, cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
			ctx.set_source_rgb(0,0,0)
			ctx.move_to(self._asc_x(ctx, self.curc),(self.curr-self.offnum+2)*self.tht+4)
		...
		if self.mtt:
			...
			sh = len(mttstr)
			ctx.rectangle(self._hex_x(ctx, self.curc)+0.5*self.tdx,(self.curr-self.offnum+3)*self.tht-6,self.tdx*(2+sh),self.tht+4) #-6
			ctx.set_source_rgba(0.9,0.95,0.95,0.85)
			ctx.fill()
			ctx.set_source_rgb(0.5,0,0)
			ctx.move_to(self._hex_x(ctx, self.curc)+1.5*self.tdx,(self.curr-self.offnum+3.5)*self.tht)
			ctx.select_font_face(self.font, cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_BOLD)
