...
import struct
from utils import *
from gtk_compat import gtk, gobject, pango, gtksourceview2
import os
from pycompat import ensure_text, ensure_bytes, exec_script
from os.path import expanduser


usegtksv2 = gtksourceview2 is not None
...
class CliWindow():
	def __init__(self, app):
		...
		runwin = gtk.Window(type=gtk.WINDOW_TOPLEVEL)
#		runwin.set_icon_from_file(os.path.join(self.app.execpath,"pixmaps/icon.png"))
	...
	def save_state(self,mode="all"):
		...
		if mode in ("files","all"):
			try:
				fs = open(os.path.join(self.snipsdir,"cli_state"),"w", encoding="utf-8")
		...
		if mode in ("snippets","all"):
			try:
				fs = open(os.path.join(self.snipsdir,"cli_snippets"),"w", encoding="utf-8")
	...
	def restore_state(self):
		# open recent files
		try:
			fs = open(os.path.join(self.snipsdir,"cli_state"),"r", encoding="utf-8")
			for l in fs:
				try:
		...
		# load snippets
		try:
			fs = open(os.path.join(self.snipsdir,"cli_snippets"),"r", encoding="utf-8")
	...
	def cli_on_open (self,wg,event,fname=None):
		...
		if fname:
			for n in fname:
				f = open(n,"r", encoding="utf-8", errors="replace")
				buf = f.read()
				if buf:
	...
	def cli_on_run (self,wg,event):
		...
		if pn != -1:
			...
			if pnb != -1:
				...
				txt = script[0].get_text(script[0].get_start_iter(),script[0].get_end_iter())
				exec_script(txt, {"app": self.app, "doc": rdoc, "hv": rdoc})
